...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Chore DB 2 account.<br>
        ...
        &mdash; Chore DB 2 Team
      `
      return sendMail({ toEmail: email, subject: 'Chore DB 2 - Password Reset', content })
    })
